      ...
      {report && (
        ...
        >
          <Card className={`overflow-hidden border-t-4 shadow-lg ${
            report.status === 'Resolved' ? 'border-t-emerald-500' :
            report.status === 'In Progress' ? 'border-t-amber-500' :
            'border-t-red-500'
          }`}>
            <CardHeader className="bg-slate-50 border-b border-slate-100">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">Submitted on {format(new Date(report.timestamp), 'MMM d, yyyy')}</p>
                </div>
                ...
                <div className="relative z-10 flex justify-between">
                  {['Received', 'In Progress', 'Resolved'].map((label, index) => {
                    ...
                    <div key={index} className="flex flex-col items-center">
                      ...
                      } ${report.status === 'Resolved' && isComplete ? '!bg-emerald-500 !border-emerald-500' : ''}`}>
